...
def create(user_data: schemas.User, db: Session,request):
    try:
        ...
        db_user = models.User(username=user_data.username, email=user_data.email,full_name=user_data.full_name,gender=user_data.gender,password_hash=Hash.get_password_hash(user_data.password))
        db.add(db_user)
        db.commit()
        db.refresh(db_user)
